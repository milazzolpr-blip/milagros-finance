...
import { User, Plus, Home, Calendar, BookOpen, BarChart3, Settings, X, Search } from "lucide-react";
...
export function Header({ onAdd, onSearch, workspaceName }) {
  const navigate = useNavigate();
  return (
    <div className="flex items-center justify-between mb-5">
      <div className="flex items-center gap-1.5" style={{ backgroundColor: C.panel, border: `1px solid ${C.border}`, borderRadius: 9999, paddingLeft: 4, paddingRight: 12, paddingTop: 4, paddingBottom: 4 }}>
        ...
        <span className="text-xs" style={{ color: C.muted, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{workspaceName || "Workspace"}</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => navigate("/app/altro")} aria-label="Profilo e impostazioni" title="Profilo e impostazioni" style={iconBtnStyle}><User size={14} style={{ color: C.muted }} /></button>
