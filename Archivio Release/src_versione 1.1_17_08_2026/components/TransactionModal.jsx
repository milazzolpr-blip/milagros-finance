...
import { ArrowUpRight, ArrowDownRight, ChevronRight, X, Camera, Banknote, CreditCard, Repeat, Wallet, Smartphone, MoreHorizontal, CalendarDays, Trash2, Paperclip } from "lucide-react";
 ...
 */
export default function TransactionModal({ workspace, member, existing, onClose, onSaved, onDeleted, zIndex }) {
  const { user } = useAuth();
  ...
  const [data, setData] = useState(existing?.date || todayLocal());
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoadingRefs(true);
      const [{ data: mem }, { data: cats }, { data: recentTx }] = await Promise.all([
  ...
  return (
    <Sheet onClose={onClose} title={isEdit ? "Modifica transazione" : "Nuova transazione"} zIndex={zIndex} right={
      <>
