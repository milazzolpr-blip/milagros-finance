...
export default function MembriTab({ workspace, currentUserId }) {
  ...
  const handleRoleChange = async (memberId, newRole, precedente) => {
    setMembers((prev) => prev.map((m) => (m.id === memberId ? { ...m, role: newRole } : m)));
    const { error } = await supabase.from("workspace_members").update({ role: newRole }).eq("id", memberId);
    if (error) {
      setMembers((prev) => prev.map((m) => (m.id === memberId ? { ...m, role: precedente } : m)));
      showToast("Cambio ruolo non riuscito: " + error.message, "error");
    }
  };

  const handleRemove = async (memberId) => {
    const { error } = await supabase.from("workspace_members").update({ status: "removed" }).eq("id", memberId);
    if (error) { showToast("Rimozione non riuscita: " + error.message, "error"); return; }
  ...
  const handleGenerateInvite = async () => {
    ...
    setGenerating(false);
    if (error) { showToast("Generazione codice non riuscita: " + error.message, "error"); return; }
    showToast("Codice invito generato");
    load();
  };

  const handleRevoke = async (inviteId) => {
    const { error } = await supabase.from("workspace_invites").update({ status: "revoked" }).eq("id", inviteId);
    if (error) { showToast("Revoca non riuscita: " + error.message, "error"); return; }
    showToast("Codice revocato");
    load();
  };
        ...
        <div className="space-y-3">
          {members.map((m) => (
            <div key={m.id} className="flex items-center gap-3">
              ...
              <select value={m.role} onChange={(e) => handleRoleChange(m.id, e.target.value, m.role)}
