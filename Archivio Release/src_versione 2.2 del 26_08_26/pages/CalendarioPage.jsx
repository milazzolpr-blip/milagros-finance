...
export default function CalendarioPage() {
  ...
  const eventiPerGiorno = (data) => {
    ...
    const list = [
      ...
      ...dati.attivita.filter((a) => a.data === data && (passaFiltro(a.chi_accompagna_id) || passaFiltro(a.chi_riprende_id))).map((a) => ({ tipo: "figlio", ora: a.ora?.slice(0, 5) || "", item: a })),
      ...dati.eventi.filter((e) => e.data === data && passaFiltro(e.member_id)).map((e) => ({ tipo: "evento", ora: e.ora_inizio?.slice(0, 5) || "", item: e })),
    ];
      ...
      {vista === "settimana" ? (
        <div className="flex items-center gap-1 mb-4">
          ...
          {giorniSettimana.map((g) => {
            ...
            return (
              <button key={g} onClick={() => { setSelectedDate(g); if (!isReader) setAddMenuDate(g); }} className="flex-1 flex flex-col items-center gap-1" style={{
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {meseGrid.celle.map((g) => {
              ...
              const haEventi = eventiPerGiorno(g).length > 0;
              return (
                <button key={g} onClick={() => { setSelectedDate(g); if (!isReader) setAddMenuDate(g); }} className="flex flex-col items-center justify-center gap-0.5" style={{
      ...
      <div className="space-y-2.5">
        {eventiOggi.map(({ tipo, ora, item }) => {
          ...
          if (tipo === "scadenza") {
            ...
            titolo = item.titolo;
            sottotitolo = [memberById[item.member_id]?.display_name, item.luogo].filter(Boolean).join(" · ");
          }
