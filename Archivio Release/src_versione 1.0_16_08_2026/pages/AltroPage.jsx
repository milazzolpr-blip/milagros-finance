...
import { LogOut, Plus, Pencil, Sun, Moon, Palette } from "lucide-react";
...
const TEMI = [
  { key: "scuro", nome: "Scuro", desc: "Sfondo nero, testo bianco ad alto contrasto", Icon: Moon },
  { key: "chiaro", nome: "Chiaro", desc: "Sfondo bianco, testo nero ad alto contrasto", Icon: Sun },
  { key: "personalizzato", nome: "Personalizzato", desc: "Definisci i tuoi colori", Icon: Palette },
];
...
export default function AltroPage() {
  ...
  return (
    <div>
      ...
      <PillTabs options={[{ key: "profilo", label: "Profilo" }, { key: "workspace", label: "Workspace" }, { key: "preferenze", label: "Preferenze" }]} value={tab} onChange={setTab} />

      {tab === "profilo" && <ProfiloTab member={member} user={user} signOut={signOut} />}
      {tab === "workspace" && workspace && <WorkspaceTab workspace={workspace} />}
...
function WorkspaceTab({ workspace }) {
  ...
  return (
    <>
      ...
      <Card eyebrow="Abbonamenti / spese ricorrenti">
        <div className="text-xs mb-3" style={{ color: C.muted }}>Generano un promemoria mensile — la generazione automatica delle transazioni è un passo successivo, per ora è solo un elenco.</div>
        {ricorrenti.map((r) => (
          <div key={r.id} className="flex items-center gap-3 mb-2.5">
            ...
            <div className="flex-1"><div className="text-sm" style={{ color: C.text }}>{r.nome}</div><div className="text-xs" style={{ color: C.muted }}>ogni mese, giorno {r.giorno}</div></div>
  ...
  return (
    <>
      ...
      <div className="text-xs mb-3" style={{ color: C.muted }}>
        Selezione visiva soltanto — cambiare davvero i colori dell'intera app è un lavoro a parte, non ancora fatto. Per ora l'app resta sempre in tema scuro indipendentemente da cosa scegli qui.
      </div>
      <div className="space-y-2 mb-4">
        {TEMI.map((t) => {
          ...
          return (
            <button key={t.key} onClick={() => setTema(t.key)} className="w-full flex items-center gap-3"
              ...
              <div style={{ width: 18, height: 18, borderRadius: 9999, border: `2px solid ${active ? C.purple : C.border}`, backgroundColor: active ? C.purple : "transparent", flexShrink: 0 }} />
            </button>
          );
        })}
      </div>
