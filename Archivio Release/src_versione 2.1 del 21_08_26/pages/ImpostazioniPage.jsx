...
function SincronizzazioneEsterna() {
  ...
  const servizi = [
    { key: "google", nome: "Google Calendar", colore: "#4285F4" },
    { key: "apple", nome: "Apple Calendario", colore: "#8b93a7" },
    { key: "outlook", nome: "Microsoft Outlook", colore: "#0078D4" },
  ];
  return (
    <Card eyebrow="Sincronizzazione calendari esterni">
      <div className="text-xs mb-3" style={{ color: C.muted }}>Collega i tuoi calendari esterni per vederli qui — l'integrazione vera arriva in un secondo momento, per ora puoi solo vedere come sarà.</div>
      <div className="space-y-2">
        {servizi.map((s) => (
          <div key={s.key} className="flex items-center gap-3">
            ...
            <button onClick={() => setMessaggio(`${s.nome}: disponibile prossimamente`)} className="text-xs font-medium" style={{ padding: "6px 12px", borderRadius: 9999, backgroundColor: C.panel2, color: C.muted, border: `1px solid ${C.border}` }}>
              Connetti
            </button>
          </div>
        ))}
      </div>
      {messaggio && <div className="text-xs mt-3" style={{ color: C.muted, textAlign: "center" }}>{messaggio}</div>}
    </Card>
  );
}
