...
import { Clock, Building2, Home as HomeIcon, CalendarDays } from "lucide-react";
      ...
      {modalitaSelezione === "intervallo" ? (
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <button onClick={() => cambiaMese(-1)} style={{ background: "none", border: "none", padding: 4 }}>‹</button>
            <span className="text-xs font-medium flex items-center gap-1" style={{ color: C.muted }}><CalendarDays size={12} /> {new Date(meseVisibile + "T00:00:00").toLocaleDateString("it-IT", { month: "long", year: "numeric" })}</span>
            <button onClick={() => cambiaMese(1)} style={{ background: "none", border: "none", padding: 4 }}>›</button>
          </div>
