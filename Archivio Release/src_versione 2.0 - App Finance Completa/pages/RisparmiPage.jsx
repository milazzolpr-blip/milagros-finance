...
export default function RisparmiPage() {
  ...
  const load = React.useCallback(() => {
    ...
    setLoadError("");
    Promise.all([
      supabase.from("conti_deposito").select("*").eq("workspace_id", workspace.id).order("created_at", { ascending: false }),
      supabase.from("deposito_movimenti").select("conto_id, importo, tipo"),
    ]).then(([contiRes, movRes]) => {
      if (contiRes.error) setLoadError(contiRes.error.message);
      const s = {};
  ...
  return (
    <div>
      ...
      {loadError && <div style={{ color: C.red, fontSize: 12, marginBottom: 12 }}>Errore: {loadError}</div>}
      ...
      {selezionato && <ContoDetailSheet conto={selezionato} workspace={workspace} onClose={() => setSelezionato(null)} onChanged={load} readOnly={isReader} />}
