...
function FigliTab({ workspace, isReader }) {
  ...
  const load = React.useCallback(() => {
    ...
    Promise.all([
      ...
      supabase.from("entita_attivita").select("*, entita_familiari(nome, icona)").eq("workspace_id", workspace.id).gte("data", todayLocal()).order("data").limit(30),
    ]).then(([entRes, attRes]) => {
  ...
  const conteggioPerEntita = {};
  attivita.forEach((a) => { conteggioPerEntita[a.entita_id] = (conteggioPerEntita[a.entita_id] || 0) + 1; });
  ...
  return (
    <>
        ...
        <>
          <Card style={{ marginBottom: 10 }}>
            <div className="flex items-center justify-between mb-3">
              <div>
                <div style={{ fontSize: 10.5, color: C.muted, fontWeight: 700 }} className="uppercase">In programma</div>
                <div className="font-bold" style={{ fontSize: 22, color: C.text }}>{attivita.length}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 10.5, color: C.muted, fontWeight: 700 }} className="uppercase">Prossima</div>
                <div className="text-sm" style={{ color: C.text }}>{attivita[0] ? `${attivita[0].titolo} · ${attivita[0].data.slice(8, 10)}/${attivita[0].data.slice(5, 7)}` : "—"}</div>
              </div>
            </div>
            <div className="flex gap-2" style={{ flexWrap: "wrap" }}>
              {entita.map((e) => (
                <div key={e.id} className="flex items-center gap-1.5" style={{ backgroundColor: C.panel2, borderRadius: 9999, padding: "5px 10px" }}>
                  <span style={{ fontSize: 12 }}>{e.icona}</span>
                  <span className="text-xs" style={{ color: C.text }}>{e.nome}</span>
                  <span className="text-xs" style={{ color: C.muted }}>· {conteggioPerEntita[e.id] || 0}</span>
                </div>
              ))}
            </div>
          </Card>
      ...
      {!isReader && (
        <>
          {entita.length > 0 && (
            <button onClick={() => setShowNuovaAttivita(true)} className="w-full flex items-center justify-center gap-2" style={{ backgroundColor: C.panel, border: `1px solid ${C.orange}`, borderRadius: 16, padding: 14, color: C.orange, marginBottom: 10, background: "none" }}>
              <Plus size={16} /><span className="text-sm font-medium">Nuova attività</span>
            </button>
          )}
      ...
      {showNuovaAttivita && (
        <AttivitaFiglioSheet workspace={workspace} onClose={() => setShowNuovaAttivita(false)} onSaved={() => { setShowNuovaAttivita(false); load(); }} />
      )}
    </>
  );
}
