...
export default function MeseDetailSheet({ workspace, mese, members, onClose, bumpRefresh, readOnly }) {
  ...
  // breakdown per modalità di pagamento, sulla stessa base filtrata per persona (entrate e uscite insieme)
  const modalitaMap = {};
  filtrate.filter((t) => t.modalita).forEach((t) => {
    if (!modalitaMap[t.modalita]) modalitaMap[t.modalita] = { entrate: 0, uscite: 0 };
    if (t.tipo === "entrata") modalitaMap[t.modalita].entrate += Number(t.importo); else modalitaMap[t.modalita].uscite += Number(t.importo);
  });
  const totaleModalitaFiltrata = Object.values(modalitaMap).reduce((s, d) => s + d.entrate + d.uscite, 0) || 1;
  const modalitaOrdinate = Object.entries(modalitaMap)
    .map(([name, d]) => ({ name, entrate: d.entrate, uscite: d.uscite, totale: d.entrate + d.uscite, pct: ((d.entrate + d.uscite) / totaleModalitaFiltrata) * 100 }))
    ...
    <Sheet onClose={onClose} title={meseLabelFull(mese)}>
      ...
      {!loading && giorni.length > 0 && (
        <div className="flex gap-2 mb-4">
          {[{ k: "giorni", l: "Giorno per giorno" }, { k: "categorie", l: "Per categoria" }, { k: "modalita", l: "Per modalità" }].map((v) => {
      ...
      {vista === "modalita" && !loading && giorni.length > 0 && (
        <div style={{ backgroundColor: C.panel, border: `1px solid ${C.border}`, borderRadius: 16, padding: 16 }}>
          {modalitaOrdinate.length === 0 && <div className="text-xs" style={{ color: C.muted }}>Nessuna transazione con modalità per questo filtro.</div>}
          {modalitaOrdinate.map((m) => (
            <div key={m.name} className="mb-3">
              <div className="flex justify-between text-xs mb-1">
                <span className="font-medium" style={{ color: C.text }}>{m.name}</span>
                <span style={{ color: m.color, fontFamily: "monospace" }}>{euroPlain(m.totale)}</span>
              </div>
              <div style={{ height: 5, backgroundColor: C.panel2, borderRadius: 4, overflow: "hidden", marginBottom: 4 }}>
                <div style={{ height: "100%", width: `${Math.min(m.pct * 2, 100)}%`, backgroundColor: m.color, borderRadius: 4 }} />
              </div>
              <div className="flex gap-3 text-xs" style={{ color: C.muted }}>
                {m.entrate > 0 && <span style={{ color: C.green }}>+{euroPlain(m.entrate)}</span>}
                {m.uscite > 0 && <span style={{ color: C.red }}>-{euroPlain(m.uscite)}</span>}
                <span>{m.pct.toFixed(0)}% del totale</span>
              </div>
            </div>
          ))}
        </div>
                  ...
                  <button key={t.id} onClick={() => setEditing(t)} className="w-full flex items-center gap-3" style={{ background: "none", border: "none", textAlign: "left", padding: 0 }}>
                    ...
                    <div className="flex-1" style={{ minWidth: 0 }}>
                      ...
                      <div className="text-xs truncate" style={{ color: C.muted }}>
                        {member && <span style={{ color: member.colore || C.purple }}>{member.display_name}</span>} · {t.macro_categoria}{t.micro_categoria ? ` · ${t.micro_categoria}` : ""}
                      </div>
                    </div>
                    <div className="flex flex-col items-end" style={{ flexShrink: 0 }}>
                      <div className="text-sm" style={{ color: t.tipo === "uscita" ? C.red : C.green, fontFamily: "monospace" }}>
                        {t.tipo === "uscita" ? "-" : "+"}{euroPlain(t.importo)}
                      </div>
                      {t.modalita && (
                        <div style={{ fontSize: 9.5, color: C.muted, backgroundColor: C.panel2, borderRadius: 9999, padding: "1px 7px", marginTop: 3, whiteSpace: "nowrap" }}>{t.modalita}</div>
                      )}
                    </div>
                  </button>
                );
